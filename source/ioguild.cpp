...
bool IOGuild::createGuild(Player* player)
{
	...
	DBQuery query;
	query << "INSERT INTO `guilds` (`id`, `world_id`, `name`, `ownerid`, `creationdata`, `motd`, `checkdata`) VALUES (NULL, " << g_config.getNumber(ConfigManager::WORLD_ID) << ", " << db->escapeString(player->getGuildName()) << ", " << player->getGUID() << ", " << time(NULL) << ", 'Sua guild foi criada com sucesso, para ver todos os comandos digite: !commands. Se você gostaria de remover está mensagem use !cleanmotd e para definir novo motd use !setmotd texto.', 0)";
	if(!db->query(query.str()))
		return false;
