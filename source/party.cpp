...
bool Party::passLeadership(Player* player)
{
	...
	char buffer[125];
	sprintf(buffer, "%s é o novo líder da party.", player->getName().c_str());
	broadcastMessage(MSG_INFO_DESCR, buffer, true);

	player->sendTextMessage(MSG_INFO_DESCR, "Você agora é o líder da party.");
	updateSharedExperience();
...
bool Party::join(Player* player)
{
	...
	sprintf(buffer, "Você se juntou %s'%s party. Abra o canal party para se comunicar com seus companheiros.", leader->getName().c_str(), (leader->getName()[leader->getName().length() - 1] == 's' ? "" : "s"));
...
bool Party::invitePlayer(Player* player)
{
	...
	sprintf(buffer, "%s convidou você para %s party.", leader->getName().c_str(), (leader->getSex(false) ? "sua" : "sua"));
	player->sendTextMessage(MSG_INFO_DESCR, buffer);
...
bool Party::setSharedExperience(Player* player, bool _sharedExpActive)
{
	...
	if(sharedExpActive)
	{
		...
		if(sharedExpEnabled)
			leader->sendTextMessage(MSG_INFO_DESCR, "Experiência compartilhada agora está ativo.");
		else
			leader->sendTextMessage(MSG_INFO_DESCR, "Experiência compartilhada foi ativado, mas alguns membros de seu party estão inativos.");
	}
	else
		leader->sendTextMessage(MSG_INFO_DESCR, "Experiência compartilhada está desativada.");
